...
import { Checkbox, Menu, Popover, Radio, Row, Tooltip } from "antd";
    ...
    const html = menuList?.map((section) => {
      return getItem(
        ...
        section.newData
          ...
          .map((lecture) => {
            console.log("duration", AuthUtils.getUserProfile);
            return getItem(
              //children title
              <div className="">
                <Row className="menu-children-item">
                  <Link
                ...
                <Row className="lecture-duration">
                  {(lecture.totalTime && formatTime(lecture.totalTime)) ||
                    "01:00"}
