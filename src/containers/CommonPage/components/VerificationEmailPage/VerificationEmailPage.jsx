...
const VerificationEmailPage = () => {
  ...
  const generateMessage = async () => {
    const strToken = location.search.substring(location.search.lastIndexOf("token=") + 6)
    ...
    try {
      ...
      if (response?.data?.isSuccess) {
        setObjectRender(generateObject(
            "success", "Thank You", "Your account has been active. You can login right now.", 'Sign in now', '/sign-in'
          ));
      }
    } catch (error) {
      if (error.response.status === 400) {
        setObjectRender(generateObject(
          "failed", "Sorry", error.response.data.message, 'Go to Homepage', '/'
        ));
        return
      }
      setObjectRender(generateObject(
        "failed", "Sorry", error.message, 'Go to Homepage', '/'
      ));
    }
  };
